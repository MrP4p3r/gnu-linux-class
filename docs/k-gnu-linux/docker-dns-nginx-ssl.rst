...
В работе рассмотрены:
    ...
    1. Настройка VDS сервера под управлением в качестве хост системы для службы Docker.
...
В наличии имеется VDS сервер, работающий на базе KVM. На сервере установлен
64-разрядная версия дистрибутива GNU/Linux CentOS 7.3. Сервер был арендован
на хостинге https://myvps.ru.
...
python в контейнере. Это заметно увеличит размер контейнера, поэтому были рассмотрены
альтернативы: supervisord (написан на GO), immortal (на GO), // TODO
Самым удобным вариантом по установке, конфигурации и управлению оказался Immortal.
...
описано `тут ../other/bind-nginx-ssl-rtd.rst`_.
...
Добавляем наш собственный домен в PowerDNS и запускаем редактирование зоны::

    $ docker exec -ti dns-nginx-ssl pdnsutil TODO
    $ docker exec -ti dns-nginx-ssl pdnsutil edit-zone hlebushe.gq

Добавляем пару NS записей, указывающих на наш сервер и поддомены для
сервисов, которые мы хотим запустить.
